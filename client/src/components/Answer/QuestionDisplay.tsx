...
import SingleNumFillIn from './SingleNumFillInDisplay'; // 单数字填空题展示组件
import MultiNumFillIn from './MultiNumFillInDisplay'; // 多数字填空题展示组件
import SingleTextFillIn from './SingleTextFillInDisplay'; // 单文本填空题展示组件
import MultiTextFillIn from './MultiTextFillInDisplay'; // 多文本填空题展示组件
import { QuestionModel } from '../../models/QuestionModel';

interface QuestionDisplayProps {
  question: QuestionModel; // 当前问题的数据
  onAnswerChange: (questionID: string, selectedOptionID: string | string[]) => void; // 用户选择答案的回调
}

const QuestionDisplay: React.FC<QuestionDisplayProps> = ({ question, onAnswerChange }) => {
  // 包装一下 onAnswerChange，传递额外的 questionID
  const handleAnswerChange = (selectedOptionID: string | string[]) => {
    onAnswerChange(question.QuestionID, selectedOptionID); // 传递 questionID 和选中的答案
  };
